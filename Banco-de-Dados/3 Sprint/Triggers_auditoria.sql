...
-- TRIGGER PARA A TABELA DE USUÁRIO
...
BEGIN
    IF INSERTING OR UPDATING THEN
        v_new_details := 'Tipo: ' || :NEW.tipo_tratamento || 
                         ', Início: ' || TO_CHAR(:NEW.dt_inicio_tratamento, 'DD/MM/YYYY') || 
    ...
    IF UPDATING OR DELETING THEN
        v_old_details := 'Tipo: ' || :OLD.tipo_tratamento || 
                         ', Início: ' || TO_CHAR(:OLD.dt_inicio_tratamento, 'DD/MM/YYYY') || 
                         ', Dentista: ' || :OLD.id_dentista || 
                         ', Usuário: ' || :OLD.id_usuario;
    END IF;
...
SET nome_usuario = 'Jota Santos da Silva',  -- Atualizando o nome para incluir sobrenome completo
    ...
    telefone_usuario = 11987654321          -- Atualizando o número de telefone
WHERE id_usuario = 65;
...
SET nome_dentista = 'Dr. Carlos Ferreira Mendes',  -- Atualizando nome completo
    ...
    telefone_dentista = 11991234567,              -- Atualizando para número celular
...
SET dt_termino_tratamento = TO_DATE('2025-03-20', 'YYYY-MM-DD'),  -- Definindo data de término
    descr_tratamento = 'Tratamento de canal no dente 26 concluído com sucesso. Paciente relatou melhora significativa.',  -- Detalhando descrição
    id_status = 2  -- Alterando status (assumindo que 2 seja "Concluído")
